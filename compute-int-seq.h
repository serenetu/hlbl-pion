...
inline double uFromTheta(const double theta)
{
  return std::cos(theta);
}

inline double thetaFromU(const double u)
{
  return std::acos(u);
}
...
inline qlat::CoordinateD etaFromParam(const qlat::CoordinateD& param, const double r0)
{
...
inline qlat::CoordinateD paramFromEta(const qlat::CoordinateD& eta, const double r0)
{
...
struct MuonLineIntegrand
{
  ...
  //
  std::vector<double> integrand(const double rp, const qlat::CoordinateD& eta) const
  {
    ...
    if (leta < 1e-10 || lc1 < 1e-10 || lc2 < 1e-10 || ls1 < 1e-10 || ls2 < 1e-10) {
      return ans;
    }
    ...
    for (int i = 0; i < 5; ++i) {
      for (int j = 0; j < 5; ++j) {
        ans[i * 5 + j] = 0.5 * factor *
          (  fc1xs[i] * fc2xs[j]
             - fs1xs[i] * fs2xs[j]
          );
        // the first 0.5 because we need to address the double counting in the subtraction term
      }
    }
    if (0.0 == x[2] && 0.0 == y[2]) {
    ...
    for (size_t i = 0; i < ans.size(); ++i) {
      if (std::isnan(ans[i])) {
        qshow::displayln(ssprintf("MuonLineIntegrand: x   =%s", show(x).c_str()));
        qshow::displayln(ssprintf("MuonLineIntegrand: y   =%s", show(y).c_str()));
        qshow::displayln(ssprintf("MuonLineIntegrand: eta =%s", show(eta).c_str()));
        for (size_t k = 0; k < ans.size(); ++k) {
          qshow::displayln(ssprintf("MuonLineIntegrand: ans[%d]=%s", k, show(ans[k]).c_str()));
        }
        assert(false);
      }
    }
  ...
  std::vector<double> integrand_eta_reflection_avg(const double rp, const qlat::CoordinateD& eta) const
  {
    using namespace qlat;
...
inline void test_computeIntSeq()
{
  ...
  DisplayInfo("", fname, "MuonLineIntegrand=%23.16e\n", mli(param)[0]);
  {
    MuonLineIntegrandPhi mliPhi;
    mliPhi.mli.init(x, y);
    ...
    DisplayInfo("", fname, "mliPhiInt=%23.16e\n", mliPhiInt);
  }
  // DisplayInfo("", fname, "mliInt=%23.16e\n", integrateMuonLine(mli));
