...
inline double specialK0(const double x)
{
...
inline double specialK1(const double x)
{
...
struct FgxxIntegrand
{
  ...
  double operator()(const double y) const
  {
    ...
    if (0.0 == y || 0.0 == len || 0.0 == ratio) {
      ...
      return y * (std::exp(- sqr(y) * ratio * len) - 1.0) * specialK0(sqr(y) * len);
    }
  }
};
...
inline double gxxCalcIntegrate(const double len, const double ratio)
{
  ...
  if (std::isnan(ret)) {
    qtimer::displayln(qtimer::ssprintf("%e %e %e %d", len, ratio, ret, std::isnan(ret)));
    assert(false);
  }
  return ret;
}

inline void recordGxx(const double len, const double lenp, const double ratio, const double gxx)
{
  qtimer::displayln(qtimer::ssprintf("gxx: %s %s %s %s",
        qtimer::show(len).c_str(), qtimer::show(lenp).c_str(), qtimer::show(ratio).c_str(),
        qtimer::show(gxx).c_str()));
}
...
inline double gxxCalc(const qlat::CoordinateD& c)
{
  ...
  if (0.0 == len) {
    ...
    if (std::isnan(ans)) {
      qtimer::displayln(qtimer::ssprintf("%e %e %e %e %d", len, ratio, std::acos(ratio), ans, std::isnan(ans)));
      assert(false);
    }
    return ans;
  }
...
inline double hxCalcIntegrate(const double len)
{
  ...
  FhxIntegrand hx;
  hx.init(len);
  const double ans = 1.0 / (4.0 * sqr(PI)) * integrate(hx, 0.0, 1.0, 0.0, 1.0e-12);
...
{
  qtimer::displayln(qtimer::ssprintf("hx: %s %s %s",
      qtimer::show(len).c_str(), qtimer::show(lenp).c_str(), qtimer::show(hx).c_str()));
}
...
inline void recordF1xx(const double len, const double lenp, const double ratio, const double f1xx)
{
  qtimer::displayln(qtimer::ssprintf("f1xx: %s %s %s %s",
        qtimer::show(len).c_str(), qtimer::show(lenp).c_str(), qtimer::show(ratio).c_str(),
        qtimer::show(f1xx).c_str()));
}
...
inline void recordG0xx(const double len, const double lenp, const double ratio, const double g0xx)
{
  qtimer::displayln(qtimer::ssprintf("g0xx: %s %s %s %s",
        qtimer::show(len).c_str(), qtimer::show(lenp).c_str(), qtimer::show(ratio).c_str(),
        qtimer::show(g0xx).c_str()));
}
...
inline void test_fderiv()
{
  ...
  for (long i = 0; i < size; ++i) {
    ...
    if (0 == qlat::get_id_node()) {
      for (int mu = 0; mu < 4; ++mu) {
        qtimer::displayln(qtimer::ssprintf("%s : %d : %20e %20e %20e", qtimer::show(c).c_str(), mu, (fds[mu] - reffds[mu]) / reffds[mu], reffds[mu], fds[mu]));
      }
    }
  }
}
...
inline void profile_gxxCalc()
{
  ...
  qtimer::DisplayInfo("", fname, "%23.16e\n", sum);
}
